...
export function useMapMarkers(map: google.maps.Map | null, options: UseMapMarkersOptions = {}) {
  const {
  ...
  const createClusters = useCallback(
    (visibleMarkers: google.maps.Marker[], radius: number): MarkerCluster[] => {
      ...
      visibleMarkers.forEach((marker) => {
        ...
        for (const [key, cluster] of clustersMap.entries()) {
          const distance = calculateDistance(lat, lng, cluster.center.lat, cluster.center.lng);
