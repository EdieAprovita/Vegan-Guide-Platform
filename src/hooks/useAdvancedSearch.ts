...
export function useAdvancedSearch() {
  ...
  const search = useCallback(async (page = 1) => {
    ...
    try {
      ...
      if (searchState.filters.resourceTypes.length === 1) {
        ...
        response = await searchByResourceType(
  ...
  const setResourceTypes = useCallback((resourceTypes: ResourceType[]) => {
    updateFilters({ resourceTypes });
  }, [updateFilters]);

  const setLocation = useCallback((location: string, coordinates?: Coordinates) => {
