 ...
 */
export async function searchUnified(params: SearchParams): Promise<BackendResponse<SearchResponse>> {
...
export async function searchByResourceType(
  ...
  return apiRequest<BackendResponse<SearchResponse>>(`/search/${resourceType}?${searchParams.toString()}`);
}
 ...
 */
export async function getSearchAggregations(filters?: Partial<SearchFilters>): Promise<BackendResponse<{
  ...
  return apiRequest<BackendResponse<{
    resourceTypes: Record<ResourceType, number>;
    locations: Array<{ name: string; count: number }>;
    priceRanges: Array<{ range: string; count: number }>;
    ratings: Record<number, number>;
